...
public class KeyValueAdaptor {
	...
	public Object valueForKeyPath( final Object target, final String keyPath ) {
		...
		switch ( keyParts.length ) {
			case 1:	
				if(keyPath.equals("timestamp")&&(target instanceof ChannelSnapshot)){					
  				    Timestamp timestamp=(Timestamp) valueForKey( target, keyPath );			
				    ChannelSnapshot channelSnapshot=(ChannelSnapshot) target;
				    int nanosecs=channelSnapshot.getNanosecs();
				    BigDecimal bigNano=new BigDecimal(nanosecs);
				    timestamp=new Timestamp(timestamp.getFullSeconds().add(bigNano.divide(new BigDecimal(1000000000))) );				  
