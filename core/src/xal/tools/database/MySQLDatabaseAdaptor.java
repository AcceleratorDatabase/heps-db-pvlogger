...
public class MySQLDatabaseAdaptor extends DatabaseAdaptor {
	...
	public Array getArray(String type, Connection connection, Object array)
		...
		// try {
		// final ArrayDescriptor descriptor = getArrayDescriptor(type,
		// connection);
		// return new ARRAY(descriptor, connection, array);
		// }
		// catch(SQLException exception) {
		// Logger.getLogger("global").log( Level.SEVERE,
		// "Error instantiating an SQL array of type: " + type, exception );
		// throw new DatabaseException("Exception generating an SQL array.",
		// this, exception);
		// }
	...
	public Connection getConnection(String urlSpec, String user, String password)
		...
		try {
			...
			Logger.getLogger("global").log(
					Level.SEVERE,
					"Error connecting to the database at URL: \"" + urlSpec
							+ "\" as user: " + user, exception);
			throw new DatabaseException(
					"Exception connecting to the database.", this, exception);
		}
	}
	 ...
	 * @throws gov.sns.tools.database.DatabaseException
	 *             if the schema fetch fails
	 */
	public List<String> fetchAllSchemas(final Connection connection)
			throws DatabaseException {
		try {
			...
			throw new DatabaseException(
					"Database exception while fetching schemas.", this,
					exception);
		}
	}
	 ...
	 */
	public ResultSet getTablesResultSet(final DatabaseMetaData metaData,
	 ...
	 */
	public ResultSet getColumnsResultSet(final DatabaseMetaData metaData,
	 ...
	 */
	public ResultSet getPrimaryKeysResultSet(final DatabaseMetaData metaData,
			final String schema, final String table) throws SQLException {
	 ...
	 * 
	 * @param type
	 *            An SQL array type * @param connection A database connection
	...
	/*private ArrayDescriptor getArrayDescriptor(final String type,
		...
		if (arrayDescriptorMap.containsKey(type)) {
			...
			ArrayDescriptor descriptor = ArrayDescriptor.createDescriptor(type,
					connection);
