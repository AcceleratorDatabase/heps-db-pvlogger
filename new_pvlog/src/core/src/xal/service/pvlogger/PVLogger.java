...
import xal.tools.database.MySQLDatabaseAdaptor;
...
public class PVLogger {
	...
	static public ConnectionDictionary newLoggingConnectionDictionary() {
		...
		return ConnectionDictionary.getInstance();
	...
	protected Connection getNewDatabaseConnection() {
		try {
		   //return _connectionDictionary.hasRequiredInfo() ? PersistentStore.connectionInstance( _connectionDictionary ) : null;
			_connectionDictionary.setDatabaseAdaptorClass(MySQLDatabaseAdaptor.class);
			_connectionDictionary.setURLSpec("jdbc:mysql://localhost:3306");
		   return PersistentStore.connectionInstance(_connectionDictionary);
		}
